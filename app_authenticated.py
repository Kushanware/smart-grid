...
def login_page():
	...
	st.set_page_config(page_title="Smart Grid - Login", layout="centered", page_icon="⚡")
	...
	with col1:
		...
		if st.button("Login", type="primary", use_container_width=True):
			if not username or not password:
				...
				success, user, role = authenticate_user(username, password)
				if success:
		...
		st.code("admin / admin123\noperator / operator123\nviewer / viewer123", language="text")
...
def admin_panel():
	...
	admin_tab1, admin_tab2, admin_tab3 = st.tabs(["Add User", "Manage Users", "Settings"])
	
	with admin_tab1:
		st.write("Create a new user account")
		add_username = st.text_input("Username", key="add_user_username")
		add_name = st.text_input("Full Name", key="add_user_name")
		add_password = st.text_input("Password", type="password", key="add_user_password")
		add_role = st.selectbox("Role", ["viewer", "operator", "admin"], key="add_user_role")
		
		if st.button("Create User", type="primary", use_container_width=True):
			if not add_username or not add_name or not add_password:
				st.error("Please fill all fields")
			else:
				success, message = register_user(add_username, add_password, add_name, add_role)
				if success:
					st.success(f"✓ {message}")
				else:
					st.error(f"❌ {message}")
			...
			for username, info in users.items():
				...
				with col3:
					new_role = st.selectbox("New role", ["viewer", "operator", "admin"], 
											index=["viewer", "operator", "admin"].index(info['role']), 
											key=f"role_{username}",
											label_visibility="collapsed")
					if new_role != info['role']:
						if st.button("Update", key=f"update_role_{username}", use_container_width=True):
							success, message = update_user_role(username, new_role)
							if success:
								st.success(message)
								st.rerun()
				with col4:
					if username != "admin" and username != st.session_state.get("username"):
						if st.button("Delete", key=f"delete_{username}", use_container_width=True):
