...
# 	df = df.copy()
# 	df["timestamp"] = pd.to_datetime(df.get("timestamp"), format="mixed")

	
# 	if "energy_kwh_cum" in df.columns and "energy_kwh" not in df.columns:
# 		df["energy_kwh"] = df["energy_kwh_cum"]
